...
- [ ] **No network calls policy**: the app makes zero outbound requests. CI fails if `fetch`, `axios`, or `XMLHttpRequest` appear anywhere in the app code (mirrors agent.md.md's automated check for the web default; enforced by `.github/workflows/ci.yml`).
